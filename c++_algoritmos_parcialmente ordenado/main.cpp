...
#include <chrono>
#include <fstream>
#include <random> // Adicionando para std::shuffle
...
void generatePartiallySortedVector(vector<int> &arr, int sortedPercentage)
{
    int sortedSize = (arr.size() * sortedPercentage) / 100;
    for (int i = 0; i < sortedSize; i++)
    {
        arr[i] = i;
    }
    for (int i = sortedSize; i < arr.size(); i++)
    {
        arr[i] = rand();
    }

    // Usando std::shuffle em vez de random_shuffle
    random_device rd;
    mt19937 g(rd());
    shuffle(arr.begin(), arr.begin() + sortedSize, g);
}
...
int main()
{
    ...
    for (int size = 50; size <= 100000; size += 50)
    {
        vector<int> arr(size);
        generatePartiallySortedVector(arr, 50);
